package com.cmmplb.security.oauth2.starter.provider.error;

import com.cmmplb.core.result.HttpCodeEnum;
import com.cmmplb.core.result.Result;
import com.cmmplb.core.result.ResultUtil;
import com.cmmplb.security.oauth2.starter.common.exceptions.OAuth2Exception;
...
@Slf4j
@SuppressWarnings({"AlibabaLowerCamelCaseVariableNaming", "rawtypes", "unchecked"})
public class GlobalWebResponseExceptionTranslator implements WebResponseExceptionTranslator<org.springframework.security.oauth2.common.exceptions.OAuth2Exception> {
    ...
    public ResponseEntity/*<org.springframework.security.oauth2.common.exceptions.OAuth2Exception>*/ translate(Exception e) {
        ...
        ase = (org.springframework.security.oauth2.common.exceptions.InvalidGrantException) throwableAnalyzer.getFirstThrowableOfType(org.springframework.security.oauth2.common.exceptions.InvalidGrantException.class,
                causeChain);
        if (null != ase) {
            if (ase.getMessage().contains("User is disabled")) {
                return handleOAuth2Exception(new InvalidGrantException(Oauth2Constants.ACCOUNT_NON_LOCKED, ase));
            }
        ...
        ase = (feign.FeignException) throwableAnalyzer.getFirstThrowableOfType(feign.FeignException.class, causeChain);
        if (ase != null) {
            return handleOAuth2Exception(new FeignException(e.getMessage().substring(e.getMessage().lastIndexOf("[")), e));
        }
        // 不包含上述异常则服务器内部错误
    ...
    // 使用自定义的异常处理类处理异常
    private ResponseEntity<Object> handleOAuth2Exception(org.springframework.security.oauth2.common.exceptions.OAuth2Exception e) {
        ...
        fail.setData(e.getOAuth2ErrorCode());
    ...
    private static class FeignException extends OAuth2Exception {

        private static final long serialVersionUID = 7503110732193705543L;

        public FeignException(String msg, Throwable t) {
            super(msg, t);
        }

        @Override
        public String getOAuth2ErrorCode() {
            return HttpCodeEnum.FEIGN_ERROR.getMessage();
        }

        @Override
        public int getHttpErrorCode() {
            return HttpCodeEnum.FEIGN_ERROR.getCode();
        }
    }
}
